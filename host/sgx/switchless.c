...
static void* _switchless_ecall_worker(void* arg)
{
    ...
    while (!context->is_stopping)
    {
        ...
        if (oe_switchless_enclave_worker_thread_ecall(
    ...
    size_t num_enclave_workers)
{
    ...
    if (num_host_workers < 0 || num_enclave_workers < 0 || enclave == NULL)
    ...
    while (tries--)
    {
        ...
        if (contexts[tries].call_arg == NULL)
        {
                    ...
                    (void* volatile*)&contexts[tries].call_arg, NULL, &args))
            {
                ...
                {
                    if (__atomic_load_n(
                            &contexts[tries].call_arg, __ATOMIC_SEQ_CST) !=
                        &args)
                        break;
